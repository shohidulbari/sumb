...
func (t *Task) IsDueToday() bool {
	return t.Deadline != nil && time.Now().Equal(*t.Deadline)
}
...
func (t *Task) GetDeadline() string {
	...
	return t.Deadline.Format("Jan 2, 2006")
}
...
func (tm *TaskManager) CreateTask(title, description string, deadline *time.Time, status TaskStatus) error {
	query := `
	INSERT INTO tasks (title, description, status, deadline, created_at, updated_at)
	VALUES (?, ?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)`

	_, err := tm.db.Exec(query, title, description, status, deadline)
	return err
}
